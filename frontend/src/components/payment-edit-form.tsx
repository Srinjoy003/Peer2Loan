...
const PaymentEditForm = ({ payment, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      const res = await fetch(`/api/payments/${payment._id || payment.id}`, {
        method: 'PUT',
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <button type="submit" disabled={loading}>Update Payment</button>
