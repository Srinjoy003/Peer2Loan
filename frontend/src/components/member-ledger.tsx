					...
					{stats.totalArrears > 0 && (
						<div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
							<p className="text-orange-700">
								...
								Outstanding arrears:{" "}
								{formatCurrency(stats.totalArrears, group.currency)}
							</p>
						</div>
					)}
					{/* Payout Schedule */}
						...
						<div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
							<p className="text-blue-700">
								{stats.payoutReceived ? (
									...
									<>
										📅 Scheduled to receive payout in Month{" "}
										{payoutCycle.cycleNumber} ({formatDate(payoutCycle.month)})
									</>
								)}
							</p>
						</div>
							...
							{memberPayments.map((payment) => {
								...
								return (
									<TableRow key={payment.id}>
										...
										<TableCell
											className={payment.penalty > 0 ? "text-orange-600" : ""}
										>
											{payment.penalty > 0
				...
				<CardContent className="space-y-2">
					<div className="grid grid-cols-2 gap-4">
						<div>
							...
							<p>
								{member.payoutAccount.accountHolderName} (
								{member.payoutAccount.accountNumber},{" "}
