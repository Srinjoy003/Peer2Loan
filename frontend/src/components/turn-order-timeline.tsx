...
export function TurnOrderTimeline({
	cycles,
	members,
	group,
}: TurnOrderTimelineProps) {
				...
				<div className="space-y-4">
					{cyclesWithRecipient.map((cycle, index) => {
						const recipient = members.find(
							(m) => m.id === cycle.payoutRecipientId
						);
						...
						return (
							<div key={key} className="flex gap-4">
								...
								<div className="flex flex-col items-center">
									<div className="flex-shrink-0">{getStatusIcon(cycle)}</div>
									{!isLast && (
								...
								<div className="flex-1 pb-8">
									...
									<div className="bg-muted rounded-lg p-3 space-y-2">
										<div className="flex justify-between">
											...
											<span>
												{cycle.potTotal && cycle.potTotal > 0
										...
										{cycle.status === "upcoming" && (
											<div className="text-muted-foreground">Upcoming turn</div>
										)}
										{cycle.notes && (
				...
				<div className="mt-6 pt-6 border-t">
					<div className="grid grid-cols-3 gap-4">
						<div className="text-center">
							...
							<p>
								{
									cyclesWithRecipient.filter((c) => c.status === "completed")
										.length
								}
							</p>
						</div>
						<div className="text-center">
							...
							<p>
								{
									cyclesWithRecipient.filter((c) => c.status === "active")
										.length
								}
							</p>
						</div>
						<div className="text-center">
							...
							<p>
								{
									cyclesWithRecipient.filter((c) => c.status === "upcoming")
										.length
								}
							</p>
						</div>
